...
while(True):
	...
	if(PublishThresholdsFlag == True):

		PubThresholdVal = PWRMeter.ReadEventsLimits()
		myAWSIoTMQTTClient.publish(ThresholdsValuesTopic, json.dumps(PubThresholdVal) , 1 )
		PublishThresholdsFlag = False

	if(PublishCalibrationFlag == True):

		PubCalibrationVal = PWRMeter.ReadCalibrationValues()
		myAWSIoTMQTTClient.publish(CalibrationValuesTopic, json.dumps(PubCalibrationVal) , 1 )
		PublishCalibrationFlag = False
...
# PWRMeter.limit.SetSagVoltage(330)
# PWRMeter.event.ScanEvents()
# PWRMeter.measure.GetValues()  