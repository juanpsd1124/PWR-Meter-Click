...
class Values():
    ...
    def __init__(self, VoltageRMS = 0 , Frequency = 0 , Temperature = 0,
                 PowerFactor = 0 , Current = 0, ActivePower = 0 , ReactivePower = 0, AppareantPower = 0 ):
...
class CalibrationValues():
    ...
    def __init__(self, VoltageCal = 0 , CurrentCal = 0 , PowerActCal = 0 , PowerReactCal = 0, LineFreqRef = 0 , ThermistorVoltage = 0):
        
        self.VoltageCal        = VoltageCal
        self.CurrentCal        = CurrentCal
        self.PowerActCal       = PowerActCal
        self.PowerReactCal     = PowerReactCal
        self.LineFreqRef       = LineFreqRef
        self.ThermistorVoltage = ThermistorVoltage
        
class EnergyCounters():
    
    def __init__(self, ImportActivePowerCounter = 0 , ImportReactivePowerCounter = 0 , ExportActivePowerCounter = 0 , ExportReactivePowerCounter = 0):
...
class EventsLimits():
    ...
    def __init__(self, VSag = 0 , VSurge = 0 , OverCurrent = 0 , OverPower = 0 , OverTemp = 0):
...
def SetCalibrationValues(VoltageCal, CurrentCal, PowerActCal, PowerReactiveCal, FreqCal):
    ...
    if( len(VoltageCal) == 5):
      ...
      numVoltageCal = int( VoltageCal.replace(".","") + "0")

    else:

      numVoltageCal = int( VoltageCal.replace(".",""))

    if( len(PowerActCal) == 5):

      numPowerActCal = int( PowerActCal.replace(".","") + "0" )
    ...
    if ( len (FreqCal) == 2):
      ...
      numFreqCal = int( FreqCal + "000" )

    else:

      numFreqCal = int( FreqCal.replace(".","") + "0" )

    
    numCurrentCal = int( CurrentCal.replace(".","") )

    numPowerReactCal = int( PowerReactiveCal.replace(".","") )
    ...
    CalibrationValArray.append( numPowerReactCal & 0xFF)
    CalibrationValArray.append( (numPowerReactCal >> 8) & 0xFF)
    CalibrationValArray.append( (numPowerReactCal >> 16) & 0xFF) 
    CalibrationValArray.append( (numPowerReactCal >> 24) & 0xFF)
    
    CalibrationFreqValArray.append( numFreqCal & 0xFF)
    CalibrationFreqValArray.append( (numFreqCal >> 8) & 0xFF)
    
    response = WriteFrame(Registers.Reg_Calibration_Current ,CalibrationValArray , 14)
    ...
    if(response[0] == 0x06 and responseFreq[0] == 0x06):

        logging.debug('Valores de calibracion ajustados. Voltage: %.2f V, Current: %.2f A, PowerAct: %.2f W, PowerReact: %.2f VaR , Frequency: %.2f Hz',
                      calibrationVal['VoltageCal'], calibrationVal['CurrentCal'], calibrationVal['PowerActCal'], calibrationVal['PowerReactCal'], calibrationVal['LineFreqRef'])
...
def ReadCalibrationValues():
    ...
    try:
        ...
        if(buffercal[0] == 0x06):
            ...
            CalValues.VoltageCal =  round( (buffercal[7] << 8 | buffercal[6] )  * 0.01, 2 )
            CalValues.CurrentCal =  round( (buffercal[5] << 24 | buffercal[4] << 16 | buffercal[3] << 8 | buffercal[2] ) * 0.01 , 2)
            CalValues.PowerActCal = round( (buffercal[11] << 24 | buffercal[10] << 16 | buffercal[9] << 8 | buffercal[8] ) * 0.01 , 2)
            CalValues.PowerReactCal = round( (buffercal[15] << 24 | buffercal[14] << 16 | buffercal[13] << 8 | buffercal[12] ) * 0.01 , 2)
            CalValues.LineFreqRef = round( (buffercal2[3] << 8 | buffercal2[2] ) * 0.001, 2) 
            ...
            CalValuesDict = {  'Success': True,
                               ...
                               'PowerReactCal': CalValues.PowerReactCal,
...
def SetEventsLimits(VSag,VSurge,OverCurrent,OverPower):
    ...
    if( len(VSurge) == 5 ):
      ...
      numVSurge = int( VSurge.replace(".","") + "0" )
    ...
    if( len(OverPower) == 5 ):
      ...
      numOverCurrent = int( OverCurrent.replace(".","") + "0" )
...
def ReadEventsLimits():
    ...
    try:
        ...
        if( bufferevent[0] == 0x06):
            ...
            Thresholds.OverTemp    = round( (bufferevent[15] << 8 | bufferevent[14] ) ,2 )
...
#SetCalibrationValues(VoltageCal, CurrentCal, PowerActCal, FreqCal):

#SetCalibrationValues("115.25", "1.16", "133.65", "5" , "60")

#SetEventsLimits("105.52","140.05","1.80","220.09")
