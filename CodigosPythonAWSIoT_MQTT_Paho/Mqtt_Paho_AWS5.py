...
import json
import threading
import CodigoPWRMeterClick14 as PWRMeter
import schedule
...
def on_subscribe(client, userdata, mid, granted_qos):
	...
	print("Subscrito a tema correctamente")
...
def SetCalibrationValuesCallback(client, userdata, message):
    ...
    if(DebugFlag == True):
    	...
    	m_out = json.loads(message.payload)
    	PWRMeter.SetCalibrationValues( m_out['VoltageCal'] , m_out['CurrentCal'] , m_out['PowerActCal'], m_out['FreqCal'])
    	upload = PWRMeter.ReadCalibrationValues()

    	print (upload)
    	print ("Se ha ejecutado Comando de ajuste valores Calibracion (Comando por ejecutar")
	...
	while(True):
		...
		if(RealTimeFlag == True):
			...
			ValuesDict = { 'voltaje':   RealTime[0] ,
                           ...
                           'potencia_activa':   RealTime[2], 
                           'potencia_reactiva': RealTime[3],
                           'potencia_aparente': RealTime[4],
                           'factor_potencia':   RealTime[7],
                           'frecuencia' : RealTime[5]  ,
                           'temperatura' : RealTime[6] ,
                           'potencia_activa_consumida' : ActPWRAccum,
                           'potencia_consumida_reactiva' : ReactPWRAccum}
